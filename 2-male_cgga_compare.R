...
metadata <- read_csv(metadata_file) |>
  select(CGGA_ID, Level_Compare, Gender) |>
  mutate(diag = Level_Compare) |>
  filter(!is.na(diag), Gender == "Male")
...
out_filtered$complete_table %>%
  ...
  write_csv("results/male_cgga_compare_dge.csv")

saveRDS(out_filtered, file = "data_store/male_cgga_compare_dge.rds")
